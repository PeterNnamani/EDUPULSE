...
export default function OnDutyWidget({ schoolId }: OnDutyWidgetProps) {
  ...
  // Assigned teacher (not roster manager): compact prompt to open duty page.
  if (!canManageRoster) {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold">You&apos;re on duty this week</h3>
          ...
          <Link
            to="/duty-attendance"
            className="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-0.5 shrink-0"
          >
            Open duty <ChevronRight className="w-3 h-3" />
          </Link>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="card">
      <div className="flex items-center justify-between mb-4">
        ...
        >
          Manage duty <ChevronRight className="w-3 h-3" />
        </Link>
      </div>
            ...
            <span
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${
                r.staff_id === user?.id
                  ...
                  : 'bg-blue-50 dark:bg-blue-900/25 text-blue-800 dark:text-blue-200'
              }`}
            >
              <UserCheck className="w-3.5 h-3.5" />
              {r.staff_name || 'Staff'}
              {r.staff_id === user?.id ? ' (you)' : ''}
            </span>
          ))}
        </div>
      )}
